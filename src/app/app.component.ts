...
@Component({
  ...
  imports: [
    ...
    LeaderboardComponent, AnalyticsPanelComponent
  ],
  template: `
    <app-toast></app-toast>
    <div class="app" [class.dark]="darkMode">
      <div class="container">
        <header>
          <div class="header-top">
            <button class="header-btn" (click)="exportCsv()" title="Export CSV">
              <span class="material-icons">download</span>
            </button>
            <button class="header-btn" (click)="showAnalytics = !showAnalytics" title="Analytics">
              <span class="material-icons">bar_chart</span>
            </button>
            <button class="header-btn" (click)="toggleDarkMode()" title="Toggle dark mode">
        ...
        <app-analytics-panel *ngIf="showAnalytics"></app-analytics-panel>
      ...
      <footer>
        <p>Built with ❤️ using Spring Boot & Angular · 5 Microservices</p>
      </footer>
    </div>
  `,
  styles: [`
    ...
    header {
      text-align: center; color: #fff; margin-bottom: 32px; position: relative;
    }
    .header-top {
    ...
    .header-btn:hover { background: rgba(255,255,255,0.25); transform: rotate(20deg); }
    .header-icon {
      font-size: 56px; margin-bottom: 8px; display: block;
      animation: float 3s ease-in-out infinite;
    }
    header h1 { font-size: 2.8rem; font-weight: 800; margin-bottom: 6px; letter-spacing: -1px; }
    .subtitle { font-size: 1.1rem; opacity: 0.85; font-weight: 300; }
...
export class AppComponent implements OnInit {
  ...
  exportCsv() {
    ...
    this.toast.show('Downloading CSV export...', 'info');
  }
}
